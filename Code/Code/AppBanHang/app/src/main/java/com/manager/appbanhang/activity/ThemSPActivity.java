...
public class ThemSPActivity extends AppCompatActivity {
    ...
    private void themSanPham() {
        ...
        int Quantity = 0;
        try {
            Quantity = Integer.parseInt(binding.txtsoluong.getText().toString().trim());
        } catch (Exception e) {
            Toast.makeText(getApplicationContext(), "Bạn phải nhập dúng kiểu dữ liệu số", Toast.LENGTH_SHORT).show();
            return;
        }
        Log.d("hinh anh :: ", str_hinhanh);
        if (str_tensp.isEmpty() || str_gia.isEmpty() || str_mota.isEmpty() || str_hinhanh.isEmpty()) {
            Toast.makeText(getApplicationContext(), "Vui lòng nhập đủ thông tin", Toast.LENGTH_SHORT).show();
        } else {
            System.out.println("data#: " + str_tensp + " , " + str_gia + " , " + str_hinhanh + " , " + str_mota + " , " + (loai) + " , " + (Quantity));
            compositeDisposable.add(apiBanHang.insertSp(str_tensp, str_gia, str_hinhanh, str_mota, loai, Quantity)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(
                            messageModel -> {
